//! Append-only on-disk block log + replay-based chain loading (P5).
//!
//! Persistence model: the block sequence IS the persisted state. The
//! log holds the non-genesis blocks (heights 1..) in order, each as a
//! `u32-LE length ‖ Block wire bytes` record in `blocks.log` under the
//! data dir; genesis is always derived from the network. Loading
...
//! block. Anything malformed *before* the tail is an error, not a
//! truncation.
...
/// Upper bound for one record's declared length: the body consensus cap
/// plus generous header/coinbase framing slack. A larger claim can only
/// be corruption — `Block::from_bytes` would reject it anyway.
...
pub enum StoreError {
    ...
    #[error("block log record at offset {offset} claims {got} bytes (> {MAX_RECORD_BYTES})")]
    RecordTooLarge { offset: usize, got: usize },
...
pub fn save_block(dir: &Path, block: &Block) -> Result<(), StoreError> {
    ...
    let mut record = Vec::with_capacity(LEN_PREFIX_BYTES + block_bytes.len());
    record.extend_from_slice(&(block_bytes.len() as u32).to_le_bytes());
    record.extend_from_slice(&block_bytes);
    let mut file = OpenOptions::new()
        .create(true)
        .append(true)
        .open(dir.join(BLOCK_LOG_FILE))?;
...
pub fn read_log(dir: &Path) -> Result<Vec<Block>, StoreError> {
    ...
    while offset < data.len() {
        ...
        if len > MAX_RECORD_BYTES {
            return Err(StoreError::RecordTooLarge { offset, got: len });
        }
    ...
    if offset < data.len() {
        tracing::warn!(
            ...
            "block log ends in a partial record (crash artifact); truncating to last complete block"
        );
