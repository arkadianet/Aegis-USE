...
| **MUST-FIX before ANY value — receipt-USE SIPHON (review-CONFIRMED critical; fires in BOTH consolidation AND payout txs — any tx spending a DepositReceipt can divert its USE):** DepositReceipt merge only checks OUTPUTS(0)=vault + PegVault only `vaultOut ≥ vaultIn` → token conservation routes the receipt USE to an attacker output. **Fix (in PegVault, not the receipt — per-receipt checks can't compose): sum USE of all consumed receipt inputs (pin by RECEIPT_SCRIPT_HASH), require `vaultOut == vaultIn + Σ(receipt USE)`.** | **theft/I1** | Pass 2 |
...
| **MUST-RESOLVE (design, cross-chain) before ANY value — refund↔mint DOUBLE-CLAIM (pass-2 review, CONFIRMED):** the refund-floor forces refund *after* N_mint, so the default ordering is MINT-then-refund: SC mints the note at N_mint but the receipt stays a live refundable box until a *permissionless* consolidation runs; a depositor front-runs consolidation → refunds AFTER being minted → keeps the private note AND reclaims the USE → inflation (bounded per-deposit N). The documented SC-used-set interlock only covers refund-FIRST, not this. **Resolution options (pin the mint↔consolidation↔refund ordering in DESIGN.md/peg.md): (a) peg-in credits the vault directly at lock (no live refundable receipt after lock; refund only if SC never mints); (b) refund gated on a proof the SC never minted; (c) SC guarantees consolidation strictly inside the margin.** NOT fixable in DepositReceipt.es alone. | **inflation** | Before value; design task |
